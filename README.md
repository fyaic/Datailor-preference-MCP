...
Runtime preference MCP hooks return a user-visible payload by default: `decision`, `injected`, the complete `agent_instruction` when one applies, and a short matched preference list containing only `id`, `title`, and `instruction`. Machine diagnostics such as `ok`, tool name, enabled state, hook name, agent, session id, `gate_summary`, scores, live confidence, gate reasons, prewarm internals, capture details, and auto-discovery file lists stay in the injection log or are available by calling the same tool with `include_debug=true`. Datailor does not truncate the visible `agent_instruction`; if an instruction is shown, it is the full executable instruction for that turn.

Runtime decisions can also return `decision="no_preference"` with `injected=false`. This is an intentional non-injection outcome, not an error. In the default visible payload, no-match decisions stay minimal so users can quickly tell that nothing was injected; use `include_debug=true` or the Manifesto Injection Log to inspect why candidates did or did not pass the gate.

Prewarm still writes session cache and fallback files, but default visible responses only surface executable fallback guidance through `fallback_instruction` / `fallback_applied` when present. Cache and fallback file paths are available with `include_debug=true`. Fallback guidance is static recovery guidance; it is not counted as dynamic preference injection, and no-match prewarm log events have `injected=false`.
