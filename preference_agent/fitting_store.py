...
class FittingJobStore:
    def __init__(self, root: str | Path | None = None):
        self.root = Path(root) if root else default_fitting_dir()
        self.jobs_dir = self.root / "jobs"

    def new_job_paths(self, job_id: str | None = None) -> FittingJobPaths:
        job_id = job_id or _new_job_id()
        job_dir = self.jobs_dir / job_id
        return FittingJobPaths(
            job_id=job_id,
            job_dir=job_dir,
    ...
    def write_result(self, result: FittingJobResult) -> None:
        _write_json(Path(result.result_file), result.to_dict())

    def write_insights(self, paths: FittingJobPaths, insights: list[InsightRecord]) -> None:
        _write_jsonl(paths.draft_insights_file, [item.to_dict() for item in insights])

    def write_rot_suggestions(self, paths: FittingJobPaths, suggestions: list[RotSuggestion]) -> None:
        _write_jsonl(paths.rot_suggestions_file, [item.to_dict() for item in suggestions])

    def write_apply_plan(self, paths: FittingJobPaths, plan: ApplyPlan) -> None:
